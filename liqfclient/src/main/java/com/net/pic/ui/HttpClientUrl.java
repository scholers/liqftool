...
public class HttpClientUrl {
		...
		if (response.getStatusLine().getStatusCode() == 200) {
			...
			for (Iterator<Element> i = es.iterator(); i.hasNext();) {
				...
				if (spanStr.indexOf("class=\"fl\" style=\"margin:0 8px\"") >= 0) {
					j++;
					message = e.text();
					System.out.println("当前净值："+ message);
	...
	private CookieStore login() throws Exception {
		...
		StringBuilder strBuild = new StringBuilder();
		/*
		 * // 得到login formhash HttpGet httpget = new HttpGet(siteLoginUrl);
		 * HttpResponse response = client.execute(httpget); HttpEntity entity =
		 * response.getEntity();
		 * 
		 * // 输出页面内容 if (entity != null) { // String charset =
		 ...
		 * new Exception("Get formhash failed!!!"); } // 找出这个 formhash
		 * 的内容，这是登录用的 formhash String loginFormhash = strBuild.substring(pos +
