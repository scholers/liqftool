...
public class Test {
    ...
    public void testArr() {
        ...
        for (int i = 0; i < tempList.size(); i++) {
            System.out.println("(TestBean)tempList.get(i)==="
                               + ((TestBean) tempList.get(i)).getName());
        }
    }
    ...
    public float computeTax(float income) //income为工资
    {
        ...
        if (i_tax_level > 0) {
            ...
            float f_tax = tax_sub[i_tax_level - 1] + (f_income - tax_line[i_tax_level - 1])
                          * tax_rate[i_tax_level]; //算税的公式
            return f_tax;
        } else {
            return 0f;
        }
    ...
    public void testDouble() {
        String sellerRate = "0.3399";
        double buyerRate = 1 - Double.parseDouble(sellerRate);
        System.out.println(String.valueOf(buyerRate));
        System.out.println(String.valueOf(round(buyerRate, 2, BigDecimal.ROUND_HALF_DOWN)));
    }
